...
class Strikes(commands.Cog, name="Strikes"):
    ...
    async def autostrikes_create(self, ctx: disnake.ApplicationCommandInteraction,
            type = commands.Param(choices=["War Hit Performance (percent)", "Capital Raid Performance (loot)", "Capital Dono Performance (loot)", "Missed War Hits (hits)",
                                          "Sat Out of War (days)", "Missed Capital Hits (hits)", "Inactivity (days)", "Clan Games (points)"]), number: str = commands.Param(name="number"),
        ...
        while not save:
            ...
            if "button" in str(res.data.component_type):
                ...
                if "th_" in res.values[0]:
                    ...
                    clan_dropdown = clan_component(bot=self.bot, all_clans=clans, clan_page=clan_page)
                    await message.edit(embed=embed, components=[clan_dropdown, th_dropdown, role_dropdown, page_buttons])
    ...
    async def add_autostrike(self, guild: disnake.Guild, autostrike_type: str, basic_filter: float, weight: int, rollover_days: int):
        pass
